--[[
    Author: tostring#1337(325428008834039811)/tostring#1677(723636701410623541)
...
local function hop() --thanks iy devs
    while wait(3) do
        local x = {}
        ...
        if #x > 0 then
            plrs.LocalPlayer:Kick("Server hopping...")
            wait()
...
spawn(function()
    if config.ServerHopAfterTime == true then
        wait(config.ServerHopTime)
        hop()
    end
end)
...
local SpamMSG = game:HttpGet("https://pastebin.com/raw/zNT3GD7x");
spawn(function()
    while wait(1) do
        --if config.ChatSpam == true then
            events.PlayerChatted:FireServer(SpamMSG, false, false, false)
        --end
    end
end)
